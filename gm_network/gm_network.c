 ...
 * @return <0: 失败
 */
static int gm_network_netlink_get_default_gateway(const char *ifname, const int family, char *gateway,
...
/**
  ...
  * @return 校验值
  */
static uint16_t gm_network_icmp_check_sum(const uint16_t *src_data, const size_t src_data_len)
{
    if (src_data == NULL)
    {
        return 0;
 ...
 * @return <0: 失败
 */
static int gm_network_ping(const int fd, const struct sockaddr *ping_addr, const uint16_t seq_num,
...
#define ICMP_CUSTOM_DATA_LEN  9
    ...
    // 校验和
    send_icmp->icmp_cksum = gm_network_icmp_check_sum((uint16_t *)send_icmp_packet, sizeof(send_icmp_packet));
...
int gm_network_get_mac_addr(const char *ifname, char *mac, const size_t mac_len)
{
    if (ifname == NULL)
    {
        return -1;
    ...
    if (mac_len < GM_NETWORK_MAC_ADDRSTRLEN)
    {
        return -3;
    }
...
int gm_network_get_ipv4_info(const char *ifname, gm_ipv4_info_t *ipv4_info, const size_t ipv4_info_count)
{
    if (ifname == NULL)
    {
        return -1;
...
int gm_network_get_ipv4_default_gateway(const char *ifname, char *gateway, const size_t gateway_len)
{
    return gm_network_netlink_get_default_gateway(ifname, AF_INET, gateway, gateway_len);
}

int gm_network_get_ipv6_info(const char *ifname, gm_ipv6_info_t *ipv6_info, const size_t ipv6_info_count)
{
    if (ifname == NULL)
    {
        return -1;
...
int gm_network_get_ipv6_default_gateway(const char *ifname, char *gateway, const size_t gateway_len)
{
    return gm_network_netlink_get_default_gateway(ifname, AF_INET6, gateway, gateway_len);
}

int gm_network_ipv4_netmask_to_prefix(const char *netmask, uint8_t *prefix)
{
    if (netmask == NULL)
    {
        return -1;
...
int gm_network_ipv4_prefix_to_netmask(const uint8_t prefix, char *netmask, const size_t netmask_len)
{
    if (prefix > 32)
    {
        return -1;
...
int gm_network_ping_host(const char *hostname, const uint8_t ping_count, const uint16_t timeout_msec)
{
    if (hostname == NULL)
    {
        return -1;
    ...
    {
        if (gm_network_ping(fd, (struct sockaddr *)&ping_addr, i, timeout_msec) != 0)
        {
            close(fd);
